...
if ($requestData['type'] == 'activePreservatives') {
    $columns = array(
            'txtPreservative','intPreservativeLifespan','	intPreservativeFreshPercentage','intPreservativeNeutralPercentage','	intPreservativeCriticalPercentage'
             );
    ...
    if(isset($_POST['search']['value']) ) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter AND ItemOnHandStatus = 1
         $query .= "
             WHERE stfPreservativeStatus = 'Active' AND (txtPreservative LIKE '%".$requestData['search']['value']."%')
             ";
    }
    ...
    while($row = mysqli_fetch_array($result)) {  // preparing an array
        ...
        $sub_array[] = '<div class="update" data-id="'.$row["intPreservativeId"].'" 	data-column="txtPreservative">' . $row["txtPreservative"] . '</div>';
    
      $sub_array[] = '<div class="update" data-id="'.$row["intPreservativeId"].'" 	data-column="intPreservativeLifespan">' . $row["intPreservativeLifespan"] . '</div>';
    
      $sub_array[] = '<div class="update" data-id="'.$row["intPreservativeId"].'" 	data-column="intPreservativeFreshPercentage">' . $row["intPreservativeFreshPercentage"] . '</div>';
    
      $sub_array[] = '<div class="update" data-id="'.$row["intPreservativeId"].'" 	data-column="intPreservativeNeutralPercentage">' . $row["intPreservativeNeutralPercentage"] . '</div>';
    
      $sub_array[] = '<div class="update" data-id="'.$row["intPreservativeId"].'" 	data-column="intPreservativeCriticalPercentage">' . $row["intPreservativeCriticalPercentage"] . '</div>';
  ...
  if(isset($_POST['search']['value']) ) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter AND ItemOnHandStatus = 1
    $query .= "
      WHERE stfPreservativeStatus = 'Inactive' AND (txtPreservative LIKE '%".$requestData['search']['value']."%')
      ";
  }
  ...
  while($row = mysqli_fetch_array($result)) {  // preparing an array
    ...
    $sub_array[] =
      '
      <button type="button" name="view" class="btn btn-sm" id="'.$row["intPreservativeId"].'" data-toggle="modal"
        data-target="#viewpreservativeModal_enable" data-id="'.$row["intPreservativeId"].'" data-prename = "'.$row["txtPreservative"].'" data-span = "'.$row["intPreservativeLifespan"].'" data-fresh ="'.$row["intPreservativeFreshPercentage"].'" data-neutral = "'.$row["intPreservativeNeutralPercentage"].'" data-critical ="'.$row["intPreservativeCriticalPercentage"].'"><i class="far fa-eye"></i> Enable</button>
  </div>
      ';
  ...
  {
  $query = "SELECT intPreservativeId, txtPreservative, intPreservativeLifespan, intPreservativeFreshPercentage, intPreservativeNeutralPercentage, intPreservativeCriticalPercentage
            FROM tblpreservatives WHERE stfPreservativeStatus = 'Inactive' ";
  $result = mysqli_query($connections, $query);
  return mysqli_num_rows($result);
  }

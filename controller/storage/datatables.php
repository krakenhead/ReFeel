...
if ($requestData['type'] == 'activeStorage') {
  $columns = array(
      'strStorageName', 'strStorageType', 'intStorageCapacity'
       );
  
    $query = "SELECT ts.intStorageId, tst.strStorageType, ts.strStorageName, ts.intStorageCapacity
            FROM tblstorage ts JOIN tblstoragetype tst ON ts.intStorageTypeId = tst.intStorageTypeId ";
  ...
  if(isset($_POST['search']['value']) ) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter AND ItemOnHandStatus = 1
     $query .= "
       WHERE stfStorageStatus = 'Active' AND (strStorageName LIKE '%".$requestData['search']['value']."%')
       ";
  }
  ...
  while($row = mysqli_fetch_array($result)) {  // preparing an array
    ...
    $sub_array[] = '<div class="update" data-id="'.$row["intStorageId"].'" 	data-column="strStorageName">' . $row["strStorageName"] . '</div>';
  
    $sub_array[] = '<div class="update" data-id="'.$row["intStorageId"].'" 	data-column="strStorageType">' . $row["strStorageType"] . '</div>';
  
    $sub_array[] = '<div class="update" data-id="'.$row["intStorageId"].'" 	data-column="intStorageCapacity">' . $row["intStorageCapacity"] . '</div>';
  ...
  {
   $query = " SELECT ts.intStorageId, tst.strStorageType, ts.strStorageName, ts.intStorageCapacity
           FROM tblstorage ts JOIN tblstoragetype tst ON ts.intStorageTypeId = tst.intStorageTypeId WHERE stfStorageStatus = 'Active' ";
...
if(isset($_POST['search']['value']) ) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter AND ItemOnHandStatus = 1
	 $query .= "
 		WHERE stfStorageStatus = 'Inactive' AND (strStorageName LIKE '%".$requestData['search']['value']."%')
 		";
}
...
while($row = mysqli_fetch_array($result)) {  // preparing an array
  ...
  $sub_array[] = '<div class="update" data-id="'.$row["intStorageId"].'" 	data-column="intStorageCapacity">' . $row["intStorageCapacity"] . '</div>';
	...
	$sub_array[] =
		'
    <button type="button" name="view" class="btn btn-sm" id="'.$row["intStorageId"].'" data-toggle="modal"
      data-target="#viewBloodStorageModal_enable" data-id="'.$row["intStorageId"].'"><i class="far fa-eye"></i> View</button>
		';

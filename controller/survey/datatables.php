...
if ($requestData['type'] == 'activeSurveyCategory') {
  $columns = array(
    ...
    $query = "SELECT intQuestionCategoryId, stfQuestionCategory, boolIsEditable
  ...
  if(isset($_POST['search']['value']) ) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter AND ItemOnHandStatus = 1
     $query .= "
       WHERE stfQuestionCategoryStatus = 'Active' AND (stfQuestionCategory LIKE '%".$requestData['search']['value']."%')
       ";
  }
  ...
  while($row = mysqli_fetch_array($result)) {  // preparing an array
    ...
    $sub_array[] = '<div class="update" data-id="'.$row["intQuestionCategoryId"].'"	data-column="stfQuestionCategory">' . $row["stfQuestionCategory"] . '</div>';
  ...
  {
   $query = " SELECT intQuestionCategoryId, stfQuestionCategory
           FROM tblquestioncategory WHERE stfQuestionCategoryStatus = 'Active' ";
  ...
  if(isset($_POST['search']['value']) ) {   // if there is a search parameter, $requestData['search']['value'] contains search parameter AND ItemOnHandStatus = 1
    $query .= "
      WHERE stfQuestionCategoryStatus = 'Inactive' AND (stfQuestionCategory LIKE '%".$requestData['search']['value']."%')
      ";
  }
  ...
  {
  $query = " SELECT intQuestionCategoryId, stfQuestionCategory
          FROM tblquestioncategory WHERE stfQuestionCategoryStatus = 'Inactive' ";
  $result = mysqli_query($connections, $query);
  return mysqli_num_rows($result);
  }
